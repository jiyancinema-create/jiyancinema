...
import { 
  ...
  SafeAreaView, Dimensions, StatusBar, RefreshControl, Modal, Pressable 
} from 'react-native';
...
export default function SeriesScreen() {
  ...
  // 1. جلب قائمة الدول ديناميكياً من مجموعة المسلسلات
  useEffect(() => {
  ...
  // 2. جلب المسلسلات (مع نظام المفضلات)
  useEffect(() => {
    setLoading(true);
    let seriesRef = collection(db, "series");
    let q;

    if (selectedCountry === 'الكل') {
      q = query(seriesRef, orderBy("createdAt", "desc"), limit(50));
    } else {
      q = query(seriesRef, where("country", "==", selectedCountry), orderBy("createdAt", "desc"));
    }
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      if (showOnlyFavorites) {
        filtered = filtered.filter(s => favorites.some(fav => fav.id === s.id));
      }
    ...
    <View style={styles.header}>
      ...
      <View style={styles.topRow}>
        ...
        <View style={styles.logoContainer}>
          <Text style={styles.logoText}>JIYAN <Text style={{color: '#FFF'}}>SERIES</Text></Text>
  ...
  const SeriesCard = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity 
        ...
        onPress={() => router.push({ pathname: '/SeriesDetails', params: { id: item.id } })}
      >
        {imgLoading && (
        ...
        <LinearGradient colors={['transparent', 'rgba(0,0,0,0.3)', 'rgba(0,0,0,0.95)']} style={styles.cardGradient}>
          <View style={styles.cardInfo}>
            <Text style={styles.sTitle} numberOfLines={1}>{item.title}</Text>
            <View style={styles.sMeta}>
              <Text style={styles.sYear}>{item.year}</Text>
...
const styles = StyleSheet.create({
  ...
  seriesCard: { width: (width / 2) - 16, height: 280, margin: 8, borderRadius: 24, overflow: 'hidden', backgroundColor: '#080808', position: 'relative' },
  ...
  cardInfo: { padding: 15 },
  sTitle: { color: '#FFF', fontSize: 15, fontWeight: 'bold', textAlign: 'right' },
  sMeta: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 },
  sYear: { color: '#888', fontSize: 12 },
